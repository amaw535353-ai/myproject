...
class SecretExposurePolicy:
    ...
    allowed_scopes_by_secret: Mapping[str, frozenset[SecretScope]]
...
def _validate_policy(policy: SecretExposurePolicy) -> None:
    required_maps = (
        policy.expected_surface_type,
        policy.expected_surface_zone,
        policy.expected_architecture_asset_by_surface,
        policy.expected_kind_by_secret,
        policy.expected_scope_by_secret,
        policy.expected_home_surface_by_secret,
        policy.minimum_sensitivity_by_secret,
        policy.expected_trust_root_by_secret,
        policy.max_rotation_age_seconds_by_secret,
        policy.allowed_target_surfaces_by_secret,
        policy.allowed_scopes_by_secret,
        policy.forbid_plaintext_by_secret,
        policy.forbid_persistent_copy_by_secret,
        policy.expected_secret_id_by_edge,
        policy.expected_endpoints_by_edge,
        policy.expected_channel_by_edge,
        policy.expected_flow_ids_by_edge,
        policy.expected_control_ids_by_edge,
    )
    if not all((policy.expected_secret_graph_id, policy.expected_secret_graph_version)):
    ...
    if not policy.entry_secret_ids <= policy.required_secret_ids or not policy.target_surface_ids <= policy.required_surface_ids:
        _reject(SecretExposureRejectReason.POLICY_INVALID, "policy entry/target scope is outside required inventory")
    if any(set(mapping) != (policy.required_surface_ids if index < 3 else policy.required_secret_ids if index < 11 else policy.required_edge_ids) for index, mapping in enumerate(required_maps)):
        _reject(SecretExposureRejectReason.POLICY_INVALID, "policy pin maps must exactly cover required inventory")
    ...
    if min(policy.max_manifest_age_seconds, policy.max_future_skew_seconds + 1, policy.max_path_hops, policy.max_paths) <= 0:
        _reject(SecretExposureRejectReason.POLICY_INVALID, "policy bounds must be positive")
...
def _validate_request(request: SecretExposureRequest, policy: SecretExposurePolicy) -> None:
    ...
    if any(not _is_sha256(actual) or actual.casefold() != expected.casefold() for actual, expected in digest_pairs):
...
def _validate_upstream(
...
) -> tuple[dict[str, object], dict[str, object], Mapping[str, ControlStatus], str]:
    ...
    if not (p7a.exact_architecture_binding_verified and p7a.mitigating_controls_visible):
    ...
    if not (posture.exact_release_identity_verified and posture.exact_upstream_evidence_binding_verified and posture.control_catalog_verified and posture.status_derived_from_evidence):
    ...
    for status, identifiers in expected_sets.items():
        if identifiers != {key for key, value in controls.items() if value == status}:
...
def _path_fact(
    ...
    unauthorized = tuple(sorted({surface_id for surface_id in surface_ids[1:] if surface_id not in policy.allowed_target_surfaces_by_secret[secret.secret_id]}))
    scope_violations = tuple(sorted({surface_id for surface_id in surface_ids[1:] if secret.authority_scope not in policy.allowed_scopes_by_secret[secret.secret_id]}))
    plaintext_edges = tuple(sorted(edge.edge_id for edge in edge_path if edge.plaintext_exposure and policy.forbid_plaintext_by_secret[secret.secret_id]))
    persistent_edges = tuple(sorted(edge.edge_id for edge in edge_path if edge.persistent_copy and policy.forbid_persistent_copy_by_secret[secret.secret_id]))
...
def _enumerate_paths(
    ...
    def walk(secret: SecretMaterial, current: str, path: tuple[SecretTransferEdge, ...], visited: frozenset[str]) -> None:
        ...
        if current in policy.target_surface_ids and path:
            ...
            return
        outgoing = [edge for edge in adjacency.get((secret.secret_id, current), ()) if edge.target_surface_id not in visited]
...
class SecretsCredentialTrustRootExposureAnalyzer:
    ...
    def evaluate(
        ...
        assets, flows, controls, architecture_sha = _validate_upstream(
            request, self._policy, architecture, p7a_assessment, p7b_assessment, p7c_assessment, posture
        )
        surfaces, secrets, edges, manifest_sha = _validate_manifest(
            manifest, request, self._policy, assets, flows, controls, architecture_sha
        )
        paths = _enumerate_paths(secrets, edges, surfaces, controls, self._policy, request.evaluated_at_epoch)
        exposed = tuple(item for item in paths if item.exposed)
        controlled = tuple(item for item in paths if not item.exposed)
        prioritized = tuple(item.path_id for item in sorted(exposed, key=lambda item: (-item.blast_radius_score, item.secret_id, item.path_id)))
        ...
        assessment_sha = hashlib.sha256(json.dumps(evidence, sort_keys=True, separators=(",", ":"), default=str).encode()).hexdigest()
