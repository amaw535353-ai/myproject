...
from aegis.incidents.lifecycle import ZERO_SHA256, IncidentLifecycleState, _validate_record
...
def _canonical_evidence(values: tuple[tuple[str, str], ...]) -> tuple[tuple[str, str], ...]:
...
def execution_identity(
    *,
    ...
    material = {
        ...
        "evidence_references": [list(item) for item in _canonical_evidence(evidence_references)],
        "governance_decision_reference": governance_decision_reference.casefold(),
        "incident_id": incident_id,
        "policy_references": list(
            _canonical_strings(policy_references, error="INCIDENT_EXECUTION_POLICY_REQUIRED")
        ),
        "requested_action": requested_action,
    }
    encoded = json.dumps(
...
def _validate_governance(record: IncidentGovernanceRecord) -> str:
    ...
    if not record.decision_sha256 or record.decision_sha256.casefold() != expected_digest:
...
def _validate_request(request: IncidentExecutionRequest) -> None:
    ...
    if (
        ...
        or request.evidence_references != governance.lifecycle.intake.evidence_references
...
class IncidentExecutionBoundary:
    ...
    def create_request(self, governance: IncidentGovernanceRecord) -> IncidentExecutionRequest:
    ...
    def execute(self, request: IncidentExecutionRequest) -> IncidentExecutionResult:
        ...
        result_json = json.dumps(
            {
                "evidence_references": [list(item) for item in result.evidence_references],
