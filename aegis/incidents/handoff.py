...
def _validate_execution_request(request: IncidentExecutionRequest) -> None:
    ...
    try:
        canonical_request = IncidentExecutionBoundary().create_request(
            request.governance
        )
    except ValueError as exc:
        if str(exc) in {
            "DENIED_GOVERNANCE_EXECUTION_REJECTED",
            "INCIDENT_EXECUTION_APPROVAL_REQUIRED",
        }:
            raise ValueError("INCIDENT_HANDOFF_APPROVED_EXECUTION_REQUIRED") from exc
        if str(exc) == "CLOSED_INCIDENT_EXECUTION_REJECTED":
            raise ValueError("CLOSED_INCIDENT_HANDOFF_REJECTED") from exc
        raise ValueError("INCIDENT_EXECUTION_REQUEST_INVALID") from exc
    except TypeError as exc:
        raise ValueError("INCIDENT_EXECUTION_REQUEST_INVALID") from exc
    if request != canonical_request:
        raise ValueError("INCIDENT_EXECUTION_REQUEST_INVALID")

    lifecycle_head = request.provenance_references[-1]
