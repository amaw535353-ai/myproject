...
|---|---|---|---|---|---|
| Indirect prompt injection | Retrieved content proposes a ticket or access-request tool and creates a synthetic server-side effect in the vulnerable path. | The same retrieved proposals reach the real hardened RAG path and are blocked by its server-owned capability policy before MCP dispatch. | [`evals/p2b_indirect_prompt_injection.py`](../evals/p2b_indirect_prompt_injection.py); [`aegis/vulnerable/indirect_prompt_injection.py`](../aegis/vulnerable/indirect_prompt_injection.py); [`aegis/rag/answering.py`](../aegis/rag/answering.py) | Vulnerable ASR 2/2; hardened ASR 0/2; FPR 0/1; SafeTaskRate 1/1. Each vulnerable success verifies the resulting ticket or pending approval side effect. | Deterministic fake-model behavior does not prove real-model behavior. The separate adaptive corpus is a non-executed catalog and is excluded from these metrics. |
...
Metrics are derived from bounded behavioral fixtures. ASR is attack success rate, FPR is false-positive rate, and SafeTaskRate is the fraction of legitimate tasks allowed. Review the [generated report](evidence/portfolio-demo-report.md) and [machine-readable sample](evidence/portfolio-demo-evidence.json). The committed sample uses `<revision>` so it cannot falsely claim to be generated from the commit that contains itself; CI preserves the unsanitized JSON and Markdown as an artifact named with the exact `${{ github.sha }}`.
...
The demo uses no network or external effects. It writes JSON and Markdown only to the requested temporary directory. Its status is computed from explicit per-case gates, and a failed gate returns a nonzero exit code after preserving the failure evidence. Regenerate the sanitized committed sample with:
