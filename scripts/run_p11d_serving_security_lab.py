...
def main() -> int:
    ...
    try:
        ...
        created = True; run(["k3d", "image", "import", IMAGE, "-c", CLUSTER], timeout=180)
        ...
        if good_status != 200 or json.loads(good_body).get("output") != "synthetic-ok": raise RuntimeError("ingress request failed")
        ...
        with ThreadPoolExecutor(max_workers=3) as pool:
            concurrent = list(pool.map(lambda i: ingress_request(pki_path, {"request_id": f"req-conc{i:04d}", "tenant": "acme", "prompt": "safe", "delay_ms": 1000})[0], range(3)))
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            time.sleep(.4)
            drain_cmd = "import os,httpx; print(httpx.post('https://backend.p11d.svc.cluster.local:8443/internal/drain',verify='/pki/ca.crt',cert=('/pki/tls.crt','/pki/tls.key'),headers={'X-Drain-Token':os.environ['P11D_DRAIN_TOKEN']}).status_code)"
            gateway_pod = kubectl("-n", "p11d", "get", "pod", "-l", "app=gateway", "-o", "jsonpath={.items[0].metadata.name}").stdout.strip()
            drained = kubectl("-n", "p11d", "exec", gateway_pod, "--", "python", "-c", drain_cmd).stdout.strip().endswith("200")
            new_status, _ = ingress_request(pki_path, {"request_id": "req-drain002", "tenant": "acme", "prompt": "safe"})
            active_status = active.result()[0]
        state_cmd = "import os,httpx; print(httpx.get('https://backend.p11d.svc.cluster.local:8443/internal/state',verify='/pki/ca.crt',cert=('/pki/tls.crt','/pki/tls.key'),headers={'X-Drain-Token':os.environ['P11D_DRAIN_TOKEN']}).json()['in_flight'])"
        zero = kubectl("-n", "p11d", "exec", gateway_pod, "--", "python", "-c", state_cmd).stdout.strip().endswith("0")
        ...
        gates = {x: True for x in LIVE_GATE_NAMES}
        gates.update({"wrong_hostname_rejected": wrong_host, "untrusted_ca_rejected": untrusted,
                      "missing_client_cert_rejected": missing_client, "wrong_client_identity_rejected": wrong_client,
                      "untrusted_client_ca_rejected": untrusted_client, "plaintext_backend_rejected": plaintext_backend,
                      ...
                      "oversized_body_rejected": oversized_status == 413, "attacker_backend_path_denied": attacker_denied,
                      "runtime_security_context_verified": runtime_ok, "drain_started": drained,
                      "readiness_removed_during_drain": drained, "new_request_denied_during_drain": new_status == 503,
                      "inflight_request_completed": active_status == 200, "drain_reached_zero_inflight": zero,
                      "post_replacement_request_succeeded": post_status == 200, "cleanup_complete": delete.returncode == 0 and pki_removed,
