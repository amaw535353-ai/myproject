...
from aegis.incidents.execution import IncidentExecutionBoundary, execution_request_digest
from aegis.incidents.governance import IncidentGovernanceBoundary, IncidentGovernanceDecision
from aegis.incidents.lifecycle import ZERO_SHA256, IncidentLifecycleBoundary, IncidentLifecycleState
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (IncidentGovernanceDecision.REQUIRES_APPROVAL, "INCIDENT_EXECUTION_APPROVAL_REQUIRED"),
    ),
)
...
def test_identity_canonicalization_contract() -> None:
    ...
    assert first.execution_identity != replace(first, incident_id="incident-abc").execution_identity
    assert (
...
def test_missing_incident_and_tampered_governance_are_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="INCIDENT_GOVERNANCE_RECORD_INVALID"):
        IncidentExecutionBoundary().create_request(replace(governance, decision_sha256="0" * 64))
...
def test_digest_preserves_opaque_case_and_action() -> None:
    ...
    assert baseline != execution_request_digest(**{**arguments, "incident_id": "Incident-001"})
