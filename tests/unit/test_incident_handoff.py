...
def test_handoff_output_is_immutable() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        handoff.execution.requested_action = "disable-credential"
    assert isinstance(handoff.evidence_references, tuple)
    assert all(isinstance(item, tuple) for item in handoff.evidence_references)
...
def test_tampered_embedded_governance_is_rejected() -> None:
    execution = _execution()
    tampered_governance = replace(
        execution.governance,
        requested_action="disable-credential",
    )

    with pytest.raises(ValueError, match="INCIDENT_EXECUTION_REQUEST_INVALID"):
        IncidentResponseHandoffBoundary().create_handoff(
            replace(execution, governance=tampered_governance)
        )


def test_governance_and_execution_projection_mismatch_is_rejected() -> None:
    execution = _execution()

    with pytest.raises(ValueError, match="INCIDENT_EXECUTION_REQUEST_INVALID"):
        IncidentResponseHandoffBoundary().create_handoff(
            replace(execution, requested_action="disable-credential")
        )
